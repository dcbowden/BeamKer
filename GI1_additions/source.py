...
def space_distribution(plot=False,save=False):
	...
	#- A small gaussian blob in the upper right------------------------------------------
	...
	if (p.type=='gauss_R_closest_smaller_kernel'):

		# I wanted to shrink the domain by /5, so
		# need to scale back up the xmax,ymax by x5.
		x0=0.06*p.xmax*5
		y0=0.02*p.ymax*5
		sigma=0.01*p.xmax
	...
	#- Single point source. -------------------------------------------------------
		...
		indices=[]
		indices.append((40,150))
		indices.append((60,140))
	...
	#- Random field plus Gaussian blob. -------------------------------------------
	...
	#- Random field from random4 without Gaussian blob. ----------------------------------------------------
	...
	#- Random field plus 2 Gaussian blob! -------------------------------------------

	elif (p.type=='random_2blob'):
	...
	if (plot or save):
		...
		for k in range(p.Nreceivers):
			...
			plt.plot((p.x[k]-p.dx/2)/1000.0,p.y[k]/1000.0,'ko',markersize='5')
			plt.text((p.x[k]-p.dy/2)/1000.0+3.0*p.dx/1000.0,p.y[k]/1000.0+3.0*p.dx/1000.0,str(k))
